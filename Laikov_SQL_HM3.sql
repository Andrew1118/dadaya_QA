...
from salary s 
left outer join employee_salary es on salary_id = s.id 
...
from salary s 
left outer join employee_salary es on salary_id = s.id 
where montly_salary <2000 and salary_id is null;
...
select role_name, s.montly_salary
from employees e 
join employee_salary es on employee_id = e.id
join salary s on salary_id = s.id 
join roles_employee re on re.employees_id = e.id 
...
full join roles_employee re on re.employees_id = es.employee_id 
...
join roles_employee re on re.employees_id = e.id 
join roles r on r.id = re.roles_id
order by employee_name, role_name, montly_salary;
